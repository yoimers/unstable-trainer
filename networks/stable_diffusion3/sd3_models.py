...
class SDTokenizer:
    ...
    def tokenize_with_weights(
        self, text: str, truncate_to_max_length=True, truncate_length=None
    ):
...
class SD3Tokenizer:
    ...
    def tokenize_with_weights(self, text: str):
        ...
        return (
            ...
            (
                self.t5xxl.tokenize_with_weights(
                    text,
                    truncate_to_max_length=False,
                    truncate_length=self.model_max_length,
                )
...
class Transformer(nn.Module):
    def __init__(self, context_size, num_layers, mode="xformers"):
        ...
        self.layers = nn.ModuleList(
            [TransformerBlock(context_size, mode) for _ in range(num_layers)]
        )
...
class MMDiT(nn.Module):
    ...
    def __init__(
        ...
        if context_processor_layers is not None:
            self.context_processor = Transformer(
                context_size, context_processor_layers, attn_mode
            )
        else:
    ...
    def forward(
        ...
        B, C, H, W = x.shape
        x = self.x_embedder(x) + self.cropped_pos_embed(H, W, device=x.device).to(
            dtype=x.dtype
        )
...
class SDClipModel(torch.nn.Module, ClipTokenWeightEncoder):
    ...
    def set_attn_mode(self, mode):
        raise NotImplementedError(
            "This model does not support setting the attention mode"
        )
...
def create_clip_l(
    ...
    if state_dict is not None:
        ...
        if "transformer.text_projection.weight" not in state_dict:
            state_dict["transformer.text_projection.weight"] = (
